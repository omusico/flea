...
if(!empty($_COOKIE['user_id'])){ 
    ...
    $dbc = connectdb();
    $query_pending_exchange = "SELECT id FROM exchange_record WHERE buyer_id='$user_obj->id' AND status='pending' ORDER BY order_time DESC";
    ...
    if($pending_num>0){
        while ($query_pending_exchange_data = mysql_fetch_array($query_pending_exchange_result)) {
            ...
            $owner_obj = new FleaUser($exchange_obj->owner_id);
                ...
                <div id="item_sub" style="border-bottom: none;">
                    ...
                    <div class="left" id="price">
                        ...
                        <div class="func_block">
                            <a onclick="send_message_box('&receiver_id=<?=$exchange_obj->owner_id?>')"><span>發訊息</span></a>
                        </div>
                    </div>
                    <div class="left" id="btns" style="width:160px;">
                        ...
                        <? if(!empty($owner_obj->contact_number)){ ?>
                        <div class="btn_item">
                            連絡電話: <?=$owner_obj->contact_number?>
                        </div>
                        <? } ?>
                        <div class="btn_item">
                            電子信箱: <?=$owner_obj->email?>
                        </div>
                    </div>
