...
if(!empty($_COOKIE['user_id'])){ 
    ...
    $dbc = connectdb();
    $query_message = "SELECT id FROM message WHERE receiver_id='$user_obj->id' ORDER BY new_reply_time DESC, send_time DESC";
    ...
    <table border="0" cellpadding="0" cellspacing="0" class="cleanTable">
        ...
        <tbody>
            ...
            while ($query_message_result_data = mysql_fetch_array($query_message_result)) {
                ...
                $unread_class = '';
                if($message_obj->is_receiver_readed==0){
                    $unread_class = 'class="unread"';
                }
            ?>
            <tr <? if($counter%2==0){ echo 'class="even"';} ?>>
                <td <?=$unread_class?>><?=$sender_obj->nickname?></td>
                <td <?=$unread_class?>><a onclick="read_message_box('&message_id=<?=$message_obj->id?>&type=receive');"><?=$message_obj->title?></a></td>
                <td <?=$unread_class?>>
                    <?
