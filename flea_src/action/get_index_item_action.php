<?
    ...
    if($category_id=='0'){
        ...
        $query_item = "SELECT bi.id,bi.item_id FROM box_item bi INNER JOIN box b ON (bi.box_id=b.id) INNER JOIN item i ON (bi.item_id=i.id) WHERE bi.type='normal' AND bi.status!='is_deleted' AND bi.status!='paid' AND b.is_released='1' AND b.is_deleted='0' AND i.category_id='$category_id' AND i.is_deleted='0' ORDER BY bi.id";
    }
    ...
    $query_item = $query_item." LIMIT $current_item,40";
    ...
    if($item_exist_num==0){
        ...
        if($item_exist_num>($current_item+40)){
        ?>
        <div id="load_more_block">
            <br class="clearboth" />
            <div class="load_more_block">
                <div class="load_more_text"><a onclick="loadItemMore('0','<?=$current_item+40?>')">看更多商品</a></div>
            </div>
        </div>
        <?
        } else {
